...
export default function Dashboard() {
    ...
    const renderLabels = () => {
        ...
        return currentData.labels.map((label, index) => (
          ...
          >
            ...
            <span className="ml-10">{label.split("(")[1].replace(")", "")}</span>{" "}
            {/* Sayıyı gösteriyoruz ve ml-4 ile küçük boşluk bırakıyoruz */}
          </li>
        ));
      };
    ...
    return (
        <div className="relative flex flex-col items-center justify-center w-full p-4 bg-white rounded-lg shadow-md">
          {/* Arka plan resmi sağ tarafta */}
          <div className="absolute inset-y-0 right-0 w-2/5 h-full z-0">
            <Image src="/bg.svg" alt="Background" layout="fill" objectFit="cover" />
          </div>
          ...
          <div className="relative z-10 w-full">
            ...
            <div className="flex flex-col lg:flex-row w-full max-w-5xl justify-start">
              {/* Grafik alanı sola kaydırıldı */}
              ...
              {/* Veri listesi sağda, aradaki boşluk azaltıldı */}
              <div className="w-full lg:w-3/5 pl-4 flex flex-col justify-center space-y-1 mt-4 lg:mt-0">
                <h2 className="text-lg font-semibold mb-4">{renderChartTitle()}</h2>
                <ul className="text-gray-700 font-semibold space-y-1">
                  {renderLabels()}{" "}
                </ul>
              </div>
            </div>
